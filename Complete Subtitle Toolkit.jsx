...
(function() {
    ...
    function runPythonTranscription() {
        ...
        var logFile = new File(Folder.temp.fsName + "/subtitle_log.txt");
        if (logFile.exists) logFile.remove();

        // === Find the bundled executable ===
        var panelFolder = File($.fileName).parent;               // folder of this JSX
        var executable = new File(panelFolder.fsName + "/subtitle_tool.exe");
        if (!executable.exists) {
            alert("subtitle_tool.exe not found.\nIt should be in the same folder as this script.\n" + executable.fsName);
            return;
        }

        // === Build the command ===
        var cmd = '"' + executable.fsName + '"' +
        ' "' + video + '"' +
        ' -o "' + outputJson + '"' +
        ' --model "' + modelDropdown.selection.text + '"';

        if (srtPath.text) cmd += ' --srt "' + srtPath.text + '"';

        // No --ffmpeg or python stuff needed – everything is inside the .exe

        // === Redirect output to a log file ===
        var platform = $.os.indexOf("Windows") >= 0 ? "win" : "unix";
        var finalCmd;
        if (platform === "win") {
            finalCmd = 'cmd.exe /c ' + '"' + cmd.replace(/"/g, '\\"') +
            ' > "' + logFile.fsName + '" 2>&1' + '"';
            // finalCmd = 'cmd /c "' + cmd + '" > "' + logFile.fsName + '" 2>&1';
        } else {
            finalCmd = 'bash -c "' + cmd.replace(/"/g, '\\"') + '" > "' + logFile.fsName + '" 2>&1';
        }

        transcribeProgress.text = "Processing... (log: " + logFile.fsName + ")";
        dialog.update();

        // === Execute ===
        try {
            var result = system.callSystem(finalCmd);
            alert(result)
        } catch (e) {
            alert("Command execution error:\n" + e.toString());
            transcribeProgress.text = "Error.";
            return;
        }

        // === Read log ===
        var logContent = "";
        if (logFile.exists) logContent = logFile.read();

        // === Load JSON ===
        var jsonFile = new File(outputJson);
        if (jsonFile.exists) {
            jsonFile.open("r");
            ...
            try {
                ...
                alert("Done! " + jsonData.subtitles.length + " subtitle lines.");
            } catch (e) {
                alert("JSON parse error:\n" + e.toString() + "\n\nLog:\n" + logContent);
            }
        } else {
            alert("JSON file not created.\n\nCommand:\n" + finalCmd + "\n\nLog:\n" + logContent);
        }
