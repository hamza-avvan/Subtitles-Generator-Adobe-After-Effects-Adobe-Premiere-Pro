...
# Then in extract_audio, use resource_path to locate ffmpeg
def extract_audio(video_path, audio_path="temp_audio.wav", ffmpeg_bin=None):
    if ffmpeg_bin is None:
        ffmpeg_bin = resource_path("ffmpeg")  # name of the bundled binary
    cmd = [ffmpeg_bin, "-y", "-i", video_path, "-vn", "-ac", "1", "-ar", "16000", audio_path]
...
def transcribe_audio(audio_path: str, model_name: str = "base"):
    # Locate the bundled ffmpeg and set PATH for Whisper
    ffmpeg_dir = os.path.dirname(resource_path("ffmpeg.exe"))
    os.environ["PATH"] = ffmpeg_dir + os.pathsep + os.environ.get("PATH", "")

    # Model directory (bundled)
    try:
        base_path = sys._MEIPASS
    except AttributeError:
        base_path = os.path.abspath(".")
...
def main():
    ...
    parser.add_argument("--ffmpeg", default=None, help="Path to ffmpeg binary (optional)")
